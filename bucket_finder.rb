...
def parse_results doc, bucket_name, host, download, depth = 0
	...
	if !doc.elements['ListBucketResult'].nil?
		puts tabs + "Bucket Found: ".colorize(:light_green).bold + bucket_name + " ( " + host + "/" + bucket_name + " )"
		...
		doc.elements.each('ListBucketResult/Contents') do |ele|
			...
			if (readable)
				if downloaded
					puts tabs + "\t" + "<Downloaded> ".colorize(:light_green).bold + url
					@logging.puts tabs + "\t" + "<Downloaded> " + url unless @logging.nil?
				else
					puts tabs + "\t" + "<Public> ".colorize(:light_cyan).bold + url
					@logging.puts tabs + "\t" + "<Public> " + url unless @logging.nil?
				end
			else
				puts tabs + "\t" + "<Private> ".colorize(:light_red).bold + url
		...
		if !err.elements['Code'].nil?
			case err.elements['Code'].text
				when "NoSuchKey"
					puts tabs + "The specified key does not exist: ".colorize(:light_red) + bucket_name
					@logging.puts tabs + "The specified key does not exist: " + bucket_name unless @logging.nil?
				when "AccessDenied"
					puts tabs + "Bucket found but access denied: ".bold + bucket_name
				...
				when "PermanentRedirect"
					if !err.elements['Endpoint'].nil?
						puts tabs + "Bucket ".bold + bucket_name + " redirects to: ".bold + err.elements['Endpoint'].text
